# ─────────────────────────────────────────────────────────────────────────────
# install-payload.sh — Sourced by setup.sh for the "payload" backend
# Expects: TARGET, SCRIPT_DIR
# Sets:    CMS_ADMIN_URL
# ─────────────────────────────────────────────────────────────────────────────

# ── 1. Next.js ────────────────────────────────────────────────
echo "▶ Creating Next.js app..."
unzip -qo "$SCRIPT_DIR/setup/cache/next.zip" -d "$TARGET/next"
if [ ! -d "$TARGET/next/node_modules" ]; then
  echo "  node_modules not in zip — running npm install..."
  (cd "$TARGET/next" && npm install)
fi

# ── 2. Install Payload ──────────────────────────────────
echo ""
echo "▶ Creating Payload CMS app..."
PAYLOAD_ZIP="$SCRIPT_DIR/setup/cache/payload.zip"
mkdir -p "$TARGET/payload"
unzip -qo "$PAYLOAD_ZIP" -d "$TARGET/payload"
if [ ! -d "$TARGET/payload/node_modules" ]; then
  echo "  node_modules not in zip — running npm install..."
  (cd "$TARGET/payload" && npm install)
fi

# ── 3. Patch Payload package.json ─────────────────────
echo ""
echo "▶ Patching Payload package.json (adding prebuild)..."
sed -i '' 's|"build": |"prebuild": "npm run generate:importmap \&\& npm run generate:types",\n    "build": |' "$TARGET/payload/package.json"

# ── 4. Patch Payload config ────────────────────────────
echo ""
echo "▶ Patching Payload config..."
PAYLOAD_CONFIG="$TARGET/payload/src/payload.config.ts"
sed -i '' "s|import { Media } from './collections/Media'|import { Media } from './collections/Media'\nimport { Pages } from './collections/Pages'|" "$PAYLOAD_CONFIG"
sed -i '' "s|export default buildConfig({|export default buildConfig({\n  serverURL: 'http://localhost:3001',\n  cors: ['http://localhost:3000'],|" "$PAYLOAD_CONFIG"
sed -i '' "s|collections: \[Users, Media\]|collections: [Users, Media, Pages]|" "$PAYLOAD_CONFIG"

# ── 5. Create Pages collection ─────────────────────────
echo "▶ Creating Pages collection..."
cat > "$TARGET/payload/src/collections/Pages.ts" << 'EOF'
import type { CollectionConfig } from 'payload'

export const Pages: CollectionConfig = {
  slug: 'pages',
  access: {
    read: () => true,
  },
  fields: [
    {
      name: 'title',
      type: 'text',
      required: true,
    },
    {
      name: 'content',
      type: 'richText',
    },
  ],
}
EOF

# ── 6. Generate import map ────────────────────────────
echo "▶ Generating Payload import map..."
(cd "$TARGET/payload" && npm run generate:importmap) || true

# ── 7. Next.js component ───────────────────────────────
echo "▶ Creating CMS example component in Next.js..."
cat > "$TARGET/next/app/cms-example.tsx" << 'EOF'
"use client";

import { useEffect, useState } from "react";

export default function CmsExample() {
  const [data, setData] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch("http://localhost:3001/api/pages")
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch from Payload API");
        return res.json();
      })
      .then(setData)
      .catch((err) => setError(err.message));
  }, []);

  return (
    <div className="p-6 bg-white rounded shadow mt-8">
      <h2 className="text-xl font-bold mb-2">Payload Pages</h2>
      {error && <div className="text-red-600">Error: {error}</div>}
      {!data && !error && <p className="text-zinc-400">Loading...</p>}
      {data?.docs?.length === 0 && (
        <p className="text-zinc-500">
          No pages yet. Create one in Payload at{" "}
          <a className="underline" href="http://localhost:3001/admin">
            localhost:3001/admin
          </a>.
        </p>
      )}
      {data?.docs?.map((page: any) => (
        <div key={page.id} className="mb-4 border-b pb-4">
          <h3 className="text-lg font-semibold">{page.title}</h3>
        </div>
      ))}
    </div>
  );
}
EOF

# ── 8. Homepage ────────────────────────────────────────
echo "▶ Updating Next.js homepage..."
cat > "$TARGET/next/app/page.tsx" << 'EOF'
import CmsExample from "./cms-example";

export default function Home() {
  return (
    <main className="min-h-screen max-w-3xl mx-auto px-6 py-16">
      <h1 className="text-3xl font-bold mb-2">My Site</h1>
      <p className="text-zinc-500 mb-8">Powered by Next.js + Payload CMS</p>
      <CmsExample />
    </main>
  );
}
EOF

# ── 9. start.sh ────────────────────────────────────────
echo "▶ Creating start.sh..."
cat > "$TARGET/start.sh" << 'EOF'
#!/bin/bash
ROOT="$(cd "$(dirname "$0")" && pwd)"

echo "▶ Starting Payload on http://localhost:3001 ..."
cd "$ROOT/payload"
npm run dev -- --port 3001 &
BACKEND_PID=$!

echo "▶ Starting Next.js on http://localhost:3000 ..."
cd "$ROOT/next"
npm run dev &
NEXT_PID=$!

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  Next.js  → http://localhost:3000"
echo "  Payload  → http://localhost:3001/admin"
echo "  Press Ctrl+C to stop."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

trap "kill $BACKEND_PID $NEXT_PID 2>/dev/null; exit" INT TERM
wait $BACKEND_PID $NEXT_PID
EOF

CMS_ADMIN_URL="http://localhost:3001/admin"

# ── Done ──────────────────────────────────────────────────────
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ✓ Setup complete!"
echo ""
echo "  To start:  bash $TARGET/start.sh"
echo ""
echo "  Next.js  → http://localhost:3000"
echo "  Payload  → $CMS_ADMIN_URL"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
