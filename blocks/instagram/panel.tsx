...
// ── Helper — numeric string round-trip ────────────────────────────────────────
...
export function InstagramPanelControls({ data, onChange }: PanelControlProps) {
  ...
  return (
    <>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              ...
              <OptionSegment
                value={str(data.columnsMedium) || "2"}
                options={COLUMN_OPTIONS}
      ...
      <PanelSection title="Buttons">
        ...
        <OptionToggle
          label="Follow button"
          checked={Boolean(data.followButton)}
          onChange={(v) => set("followButton", v)}
        />
        {Boolean(data.followButton) && (
          <OptionText
            label="Follow button text"
            value={str(data.followButtonText)}
            onChange={(v) => set("followButtonText", v)}
      ...
      <PanelSection title="Colors">
        ...
        {Boolean(data.followButton) && (
          <>
            <OptionColor
              label="Follow: text"
              value={str(data.followBtnColor)}
              onChange={(v) => set("followBtnColor", v)}
            />
            <OptionColor
              label="Follow: background"
              value={str(data.followBtnBgColor)}
              onChange={(v) => set("followBtnBgColor", v)}
            />
          </>
        )}
        {!str(data.loadMore).match(/^button$/) && !data.followButton && (
          <p className="text-xs text-zinc-400">
            Enable Load More or Follow button above to reveal colour options.
          </p>
        )}
      </PanelSection>
