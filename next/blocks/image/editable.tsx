...
export function ImageEditable({ data, onUpdate, blockId }: EditableProps) {
    ...
    return (
      <>
        <div className="group relative">
          ...
          >
            {t("editable.change", "Change image")}
          </button>
        </div>
        <MediaPickerDialog
          open={pickerOpen}
    ...
    return (
      <>
        <div className="flex min-h-44 w-full flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-zinc-300 bg-zinc-50 p-6">
          <span className="text-sm font-medium text-zinc-500">{t("editable.add", "Add an image")}</span>
          <div className="flex gap-2">
            ...
            >
              {t("editable.mediaLibrary", "Media Library")}
            </button>
            <button
              type="button"
              onClick={() => setMode("url")}
  ...
  return (
    <div className="flex min-h-44 w-full flex-col items-center justify-center gap-3 rounded-lg border-2 border-dashed border-zinc-300 bg-zinc-50 p-6">
      <span className="text-sm font-medium text-zinc-500">{t("editable.enterUrlHeading", "Enter image URL")}</span>
      <input
        type="url"
        ...
        placeholder={t("editable.urlExample", "https://example.com/image.jpg")}
        onBlur={(e) => {
          const value = e.target.value.trim();
          if (!value) { setMode("choose"); return; }
          if (!isValidImageUrl(value)) {
            setUrlError(t("editable.urlError", "Only https:// and http:// URLs are allowed."));
            return;
          }
          setUrlError(null);
